...
def stock_query(date,open,high,low,close,diff,vol):
    conn = sqlite3.connect("stock_kind.db", isolation_level=None)
    c = conn.cursor()
    # DB 연결

    c.execute("CREATE TABLE IF NOT EXISTS STOCK_KIND "
              "(CODE text PRIMARY KEY, SECOND_CODE integer, PRICE integer, NAME text)")
    # sql문 실행 - 테이블 생성

    c.execute("INSERT OR IGNORE INTO STOCK_KIND VALUES( ?, ?, ?, ?)", (code, secondCode, stdPrice, name))
